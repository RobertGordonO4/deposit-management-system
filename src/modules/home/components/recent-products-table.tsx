...
export function RecentProductsTable({ products, isLoading }: RecentProductsProps) {
  return (
    <Section title="Recent products">
      <div className="flex flex-col">
        {isLoading ? (
            ...
            <div key={product.id} className="flex items-center justify-between py-3">
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-xs text-muted-foreground">
                  {formatVolume(product.volume)} • {formatDeposit(product.deposit)} deposit •{' '}
          ...
          <div className="py-3 text-center text-muted-foreground">No products found</div>
        )}
      </div>
    </Section>
  )
